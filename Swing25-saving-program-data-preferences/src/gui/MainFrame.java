...
//        }
        ...
        prefsDialog = new PreferencesDialog(MainFrame.this);
        prefs = Preferences.userRoot().node("app-prefs");
        ...
        // Implementing listener on Pref Dialog for click event
        prefsDialog.setButtonClickListener(this::saveDataPrefs);

        // Get Default preferences and pass it to the dialog
        fetchPreferencesAndLoadDialog(prefs);
    ...
    private JMenuBar createMenuBar() {
        ...
        prefMenuItem.addActionListener((ae) -> {
    ...
    private void saveDataPrefs(DbConnectionProfile profile) {
        System.out.println(profile);
        prefs.put("username", profile.getUsername());
        prefs.put("password", profile.getPassword());
        prefs.putInt("port", profile.getPort());
    }

    private void fetchPreferencesAndLoadDialog(Preferences prefs) {
        String username = prefs.get("username", "");
        String password = prefs.get("password", "");
        Integer port = prefs.getInt("port", 0);
        if (!username.trim().isEmpty() && !password.trim().isEmpty() && port != 0) {
            prefsDialog.setDefaultProfile(new DbConnectionProfile(username, password, port));
        }
    }
}
